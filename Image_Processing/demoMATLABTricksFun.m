...
figure, imshow(threeboxes); % Can't quite see the labeled image...

% 'color' the labeled image
rgb_img = label2rgb(threeboxes, 'jet', 'k'); 
figure, imshow(rgb_img); % now much better!
...
fh2 = figure; figure, imshow(rgb_img); % Where are all my annotations?
...
size(annotated_img)
size(rgb_img)
fh3 = figure; figure, imshow(annotated_img); % The annotations along with the image are saved!

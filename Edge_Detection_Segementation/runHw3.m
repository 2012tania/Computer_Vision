...
thresholds = [0.085,0.12,0.12];

for i = 1:length(img_list)
    fh = figure;    
    img = imread([img_list{i} '.png']);
    %subplot(1, 2, 1); imshow(img);title([img_list{i} ' Image'])
    edge_img = edge(img,'canny',thresholds(i));
    %subplot(1, 2, 2); imshow(edge_img);title([img_list{i} ' Output'])
    imshow(edge_img);
    saveas(fh, ['edge_' img_list{i}]);
